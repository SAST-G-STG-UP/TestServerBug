...
Strophe = {
    ...
    NS: {
        ...
        TIMESTAMP: "jabber:x:delay",
        HISTORY: "x:xmpp:tm:history",
        VCARD: "vcard-temp",
    ...
    getElementsByTagName: function (elem, nodeName)
    {
...
Strophe.Connection.prototype = {
    ...
    connect1: function (jid, token, callback, inactivity, wait, hold, wind)
    {
      this.jid = jid;
      this.token = token;
      ...
      this._requests.push(
        new Strophe.Request(
          body.tree(),
          this._onRequestStateChange.bind(this).prependArg(this._connect_cb1.bind(this)),
     ...
     */
    connect: function (jid, pass, callback, inactivity, wait, hold, wind)
    {
        this.jid = jid;
        this.pass = pass;
        this.connect_callback = callback;
        ...
        this._requests.push(
          new Strophe.Request(
            body.tree(),
            this._onRequestStateChange.bind(this).prependArg(this._connect_cb.bind(this)),
            body.tree().getAttribute("rid")
          )
        );
        this._throttledRequestHandler();
    ...
    {
        try { console._log('recv << ' + data) } catch (err) {}
        return;
    },
    ...
    {
        try { console._log('sent >> ' + data) } catch (err) {}
        return;
    },
    ...
    */
    ...
    _connect_cb1: function (req)
    {
      ...
      if (mechanisms.length == 0) {
        mechanisms = Strophe.getElementsByTagName(bodyWrap, "mechanism");
      }
      ...
      if (do_sasl_digest_md5) {
        ...
        var body = $build("x-tmtoken", {xmlns: Strophe.NS.SASL}).t(this.token);
        this.send($build("x-tmtoken", {xmlns: Strophe.NS.SASL}).t(this.token).tree());
      } else {
        this._changeConnectStatus(Strophe.Status.AUTHENTICATING, null);
        ...
        this.send(
          ...
          .c("query", {xmlns: Strophe.NS.AUTH}).c("username", {})
    ...
    _sasl_bind_cb: function (elem)
    {
        ...
        if (bind.length == 0) {
          bind = Strophe.getElementsByTagName(elem, "bind")
        }
        var jidNode;
        if (bind.length > 0) {
            // Grab jid
            ...
            if (jidNode.length == 0) {
              jidNode = Strophe.getElementsByTagName(bind[0], "jid")
            }
            if (jidNode.length > 0) {

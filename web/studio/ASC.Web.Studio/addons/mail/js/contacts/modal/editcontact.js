...
window.editContactModal = (function ($) {
    ...
    function show(contact, isNew) {
        ...
        html.find('.contactDescription').off('keyup').on('keyup', function() {
            if (this.value.length > 100)
                this.value = this.value.substr(0, 100);
        });

        html.find('.addEmail').off('click').on('click', addEmail);
        html.find('.contactEmails .delete_entity').off('click').on('click', deleteEmail);
        html.find('.addPhone').off('click').on('click', addPhone);
        html.find('.contactPhones .delete_entity').off('click').on('click', deletePhone);
        html.find('.save').off('click').on('click', function() { saveContact(isNew); });
        html.find('.cancel').off('click').on('click', window.PopupKeyUpActionProvider.CloseDialog);
        html.find('.contactEmail').off("keyup").on("keyup", function (e) {
            if (e.which == 13) {
                addEmail();
            } else {
        ...
        html.find('.contactEmail').off("blur").on("blur", function (e) {
        ...
        html.find('.contactPhone').off("keyup").on("keyup", function (e) {
            if (e.which == 13) {
                addPhone();
            } else {
    ...
    function addEmail() {
        ...
        var $html = $(html);
        $html.find('.delete_entity').off('click').on('click', deleteEmail);
        ...
        $('#mail-contact-edit').find('.contactEmail').trigger("focus");
        updateEmailList();
    }
    ...
    function addPhone() {
        ...
        var $html = $(html);
        $html.find('.delete_entity').off('click').on('click', deletePhone);
        ...
        $('#mail-contact-edit').find('.contactPhone').trigger("focus");
    }

...
namespace ASC.Web.People
{
    public class BirthdaysNotifyClient
    {
            ...
            try
            {
                ...
                using (var db = new DbManager("default", 300))
                {
                    var q = new SqlQuery("core_user u")
                        ...
                        .Where("date_format(u.bithdate, '%m%d')", scheduleDate.ToString("MMdd"))
                        .Where("u.removed", 0)
                ...
                foreach (var tenant in tenants)
                {
                    ...
                    if (!WebItemSecurity.GetSecurityInfo(WebItemManager.PeopleProductID.ToString()).Enabled)
                    {
                        continue;
                    }
                    ...
                    {
                        if (WebItemManager.Instance[WebItemManager.PeopleProductID].IsDisabled(user.ID))
                        {
                            continue;
                        }
        ...
        {
            public BirthdaysNotifySource() : base(WebItemManager.BirthdaysProductID)
            {
